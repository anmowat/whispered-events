...
1. "audience" (0.0–1.5): how the event's stated Audience and Type aligns with the attendee's Function. Seniority is handled separately — score function fit only. LITERAL function naming (or strict C-suite alias) is the only path to 1.2+. Broad-role aliases are soft signal. Use this rubric — do not invent intermediate logic:
   ...
   Operations ≠ the underlying function: "Sales Operations", "Marketing Operations", "Revenue Operations", "Customer Success Operations" are OPERATIONS/ENABLEMENT roles — they are NOT the same as Sales, Marketing, Revenue, or CS. This cuts BOTH ways:
     • A Sales attendee at a "Sales Operations Leaders" event → 0.0. Sales Ops ≠ Sales.
     • A Marketing attendee at a "Marketing Operations Leaders" event → 0.0. Marketing Ops ≠ Marketing.
     • A Marketing Operations attendee at a "Marketing Leaders" event → 0.0. Marketing Ops ≠ Marketing.
     • A Sales Operations / RevOps attendee at a "Sales Leaders" event → 0.0. RevOps ≠ Sales.
   The word "Sales" inside "Sales Operations" does NOT make it a Sales match. The word "Marketing" inside "Marketing Operations" does NOT make it a Marketing match. These are distinct disciplines. Score them only when the audience EXPLICITLY names their function (e.g. "RevOps Leaders", "Sales Operations Leaders", "Marketing Operations Leaders" for a RevOps/SalesOps/MktgOps attendee respectively).
