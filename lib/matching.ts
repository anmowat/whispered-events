...
1. "audience" (0.0–1.5): how the event's stated Audience and Type aligns with the attendee's Function. Seniority is handled separately — score function fit only. LITERAL function naming (or strict C-suite alias) is the only path to 1.2+. Broad-role aliases are soft signal. Use this rubric — do not invent intermediate logic:
   ...
   Operations ≠ the underlying function, but they are adjacent (score 0.6, not 1.2+): "Sales Operations", "Marketing Operations", "Revenue Operations", "Customer Success Operations" are OPERATIONS/ENABLEMENT roles — distinct from but adjacent to Sales, Marketing, Revenue, CS. This cuts both ways:
     • A Sales attendee at a "Sales Operations Leaders" event → 0.6. Related but not the named audience.
     • A Marketing attendee at a "Marketing Operations Leaders" event → 0.6. Related but not the named audience.
     • A Marketing Operations attendee at a "Marketing Leaders" event → 0.6. Related but not the named audience.
     • A Sales Operations / RevOps attendee at a "Sales Leaders" event → 0.6. Related but not the named audience.
   Never score these 1.2 or higher — the word "Sales" inside "Sales Operations" does not make it a literal Sales match. Cap at 0.6 (adjacent, same family) not 0.0 (wrong audience). Score 0.6 only when the audience EXPLICITLY names the related function; if there's no naming relationship at all, it's still 0.0.
