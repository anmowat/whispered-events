...
// topics:
//   id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
//   name text UNIQUE NOT NULL,            -- short interest tag (e.g. 'RevOps')
//   sort_order integer NOT NULL DEFAULT 0,-- ascending; ties broken by name
//   created_at timestamptz NOT NULL DEFAULT now()
//   Used by the admin Topics tab and (next) the signup-flow interest
//   chips. Admin is the only writer.
...
// ---------------- Topics ----------------
// Curated list of interest tags shown as chips in the signup flow and
// editable by admin via /admin/topics. Sorted by sort_order asc, name
// asc as a tiebreaker.
...
export async function createTopic(name: string): Promise<Topic | null> {
...
// when admin reorders via the up/down arrows on /admin/topics.
export async function reorderTopics(orderedIds: string[]): Promise<boolean> {
  if (orderedIds.length === 0) return true
  const supabase = getClient()
  // Upsert each row with its new sort_order. Done in parallel — Supabase's
  // PostgREST handles concurrent updates on the same table fine here.
